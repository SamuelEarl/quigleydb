# minimal RocksDB-like KV store in Mojo

This can be found in the "Graph Database System Architecture" chat in ChatGPT.

This is a minimal RocksDB-like KV store in Mojo provided by ChatGPT (with WAL + Memtable + flush to SSTable), so you get a “Hello World LSM-tree” to start from. It includes:

* WAL (append + replay + reset)
* MemTable (in-memory map with sorted flush)
* SSTable writer/reader (naive binary format + in-memory index)
* Two-table compaction (merge)
* DB façade with put/get/delete, flush, compact_all
* An example `example()` showing basic usage
...
fn example():
    var db = DB.open("/data", memtable_flush_threshold=4)
    db.put("alice", "A")
    db.put("bob", "B")
    db.put("carol", "C")
